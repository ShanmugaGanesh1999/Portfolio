# Design Deployment System

> CI/CD pipeline for reliable software deployments

---

## 📋 Problem Statement

Design a deployment system like Kubernetes/Spinnaker that handles building, testing, and deploying software to production with safety and reliability.

---

## R - Requirements

### Functional Requirements

```
1. Build applications from source code
2. Run automated tests
3. Deploy to multiple environments (dev, staging, prod)
4. Rolling deployments with health checks
5. Rollback on failure
6. Blue-green and canary deployments
7. Configuration management
8. Deployment history and audit trail
```

### Non-Functional Requirements

```
1. High reliability (deployments never leave system broken)
2. Fast rollbacks (<5 minutes)
3. Zero-downtime deployments
4. Handle 1000s of deployments/day
5. Support multiple cloud providers
```

---

## H - High-Level Design

```
┌─────────────────────────────────────────────────────────────┐
│                                                              │
│   Developer                                                  │
│   ┌───────────┐                                             │
│   │ Git Push  │                                             │
│   └─────┬─────┘                                             │
│         │ Webhook                                            │
│         ▼                                                    │
│   ┌──────────────────────────────────────────────────────┐  │
│   │                  CI/CD Pipeline                       │  │
│   │  ┌────────┐  ┌──────┐  ┌──────┐  ┌────────────────┐ │  │
│   │  │ Build  │→ │ Test │→ │Publish│→ │    Deploy      │ │  │
│   │  │        │  │      │  │ Image │  │                │ │  │
│   │  └────────┘  └──────┘  └───────┘  └───────┬────────┘ │  │
│   └─────────────────────────────────────────────┼────────┘  │
│                                                  │           │
│   ┌──────────────────────────────────────────────▼────────┐ │
│   │               Deployment Orchestrator                  │ │
│   │  ┌──────────────────────────────────────────────────┐ │ │
│   │  │  Strategy: Rolling │ Blue-Green │ Canary         │ │ │
│   │  └──────────────────────────────────────────────────┘ │ │
│   └────────────────────────────┬──────────────────────────┘ │
│                                │                             │
│   ┌────────────────────────────▼─────────────────────────┐  │
│   │               Target Infrastructure                   │  │
│   │  ┌────────┐  ┌────────┐  ┌────────┐  ┌────────┐     │  │
│   │  │ Node 1 │  │ Node 2 │  │ Node 3 │  │ Node N │     │  │
│   │  └────────┘  └────────┘  └────────┘  └────────┘     │  │
│   └──────────────────────────────────────────────────────┘  │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

---

## D - Detailed Design

### Build Pipeline

```
┌─────────────────────────────────────────────────────────────┐
│              Build Stage                                     │
│                                                              │
│   Trigger: Git push or manual                               │
│                                                              │
│   Steps:                                                     │
│   1. Checkout code                                          │
│   2. Install dependencies                                   │
│   3. Compile/build                                          │
│   4. Run unit tests                                         │
│   5. Static analysis (linting, security scan)              │
│   6. Build container image                                  │
│   7. Push to registry                                       │
│   8. Update deployment manifest                             │
│                                                              │
│   Build specification (e.g., pipeline.yaml):                │
│   ```yaml                                                   │
│   stages:                                                   │
│     - name: build                                           │
│       image: node:18                                        │
│       commands:                                             │
│         - npm install                                       │
│         - npm run build                                     │
│         - npm test                                          │
│                                                              │
│     - name: docker                                          │
│       commands:                                             │
│         - docker build -t app:$COMMIT_SHA .                │
│         - docker push registry/app:$COMMIT_SHA            │
│   ```                                                       │
│                                                              │
│   Artifacts:                                                 │
│   ├── Container image with commit SHA tag                  │
│   ├── Build logs                                           │
│   ├── Test results                                         │
│   └── Security scan report                                 │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Deployment Strategies

```
┌─────────────────────────────────────────────────────────────┐
│              Rolling Deployment                              │
│                                                              │
│   Gradually replace old pods with new                       │
│                                                              │
│   Time →                                                    │
│   ┌────────────────────────────────────────────────────┐   │
│   │ v1 │ v1 │ v1 │ v1 │ v1 │  Initial                  │   │
│   └────────────────────────────────────────────────────┘   │
│   ┌────────────────────────────────────────────────────┐   │
│   │ v2 │ v1 │ v1 │ v1 │ v1 │  Replace 1st             │   │
│   └────────────────────────────────────────────────────┘   │
│   ┌────────────────────────────────────────────────────┐   │
│   │ v2 │ v2 │ v1 │ v1 │ v1 │  Replace 2nd             │   │
│   └────────────────────────────────────────────────────┘   │
│   ┌────────────────────────────────────────────────────┐   │
│   │ v2 │ v2 │ v2 │ v2 │ v2 │  Complete                │   │
│   └────────────────────────────────────────────────────┘   │
│                                                              │
│   Parameters:                                                │
│   ├── maxUnavailable: 25% (min available during update)   │
│   ├── maxSurge: 25% (extra pods during update)            │
│   └── minReadySeconds: 30 (wait before marking healthy)   │
│                                                              │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│              Blue-Green Deployment                           │
│                                                              │
│   Two identical environments, switch traffic instantly      │
│                                                              │
│                     Load Balancer                            │
│                          │                                   │
│              ┌───────────┴───────────┐                      │
│              ▼                       ▼                       │
│   ┌──────────────────┐    ┌──────────────────┐              │
│   │   Blue (v1)      │    │   Green (v2)     │              │
│   │   ← LIVE         │    │   ← STAGED       │              │
│   │   ┌────┐ ┌────┐  │    │   ┌────┐ ┌────┐  │              │
│   │   │ v1 │ │ v1 │  │    │   │ v2 │ │ v2 │  │              │
│   │   └────┘ └────┘  │    │   └────┘ └────┘  │              │
│   └──────────────────┘    └──────────────────┘              │
│                                                              │
│   Steps:                                                     │
│   1. Deploy v2 to Green (inactive)                         │
│   2. Test Green environment                                │
│   3. Switch LB to point to Green                           │
│   4. Green is now live, Blue is standby                    │
│   5. If problems: instant rollback (switch back)           │
│                                                              │
│   Pros: Instant rollback, clean separation                  │
│   Cons: 2× infrastructure cost                              │
│                                                              │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│              Canary Deployment                               │
│                                                              │
│   Route small % of traffic to new version                   │
│                                                              │
│   Traffic split:                                             │
│                                                              │
│   100% → v1                    (Start)                      │
│    95% → v1, 5% → v2           (Canary start)               │
│    80% → v1, 20% → v2          (Increase if healthy)        │
│    50% → v1, 50% → v2          (Continue)                   │
│     0% → v1, 100% → v2         (Complete rollout)           │
│                                                              │
│   Automated analysis:                                        │
│   ├── Compare error rates (v1 vs v2)                       │
│   ├── Compare latency percentiles                          │
│   ├── Compare resource usage                               │
│   └── If v2 worse → auto rollback                          │
│                                                              │
│   Configuration:                                             │
│   ```yaml                                                   │
│   strategy:                                                  │
│     type: canary                                            │
│     steps:                                                   │
│       - weight: 5                                           │
│         pause: {duration: 10m}                              │
│       - weight: 20                                          │
│         pause: {duration: 10m}                              │
│       - weight: 50                                          │
│         pause: {duration: 10m}                              │
│       - weight: 100                                         │
│   ```                                                       │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Health Checks

```
┌─────────────────────────────────────────────────────────────┐
│              Health Checking                                 │
│                                                              │
│   Types:                                                     │
│                                                              │
│   1. Liveness probe: Is the app alive?                      │
│      - HTTP GET /healthz returns 200                       │
│      - If fails → restart container                        │
│                                                              │
│   2. Readiness probe: Is the app ready for traffic?        │
│      - HTTP GET /ready returns 200                         │
│      - If fails → remove from load balancer               │
│                                                              │
│   3. Startup probe: Has the app started?                   │
│      - For slow-starting apps                              │
│      - Delays liveness checks until startup complete       │
│                                                              │
│   Configuration:                                             │
│   ```yaml                                                   │
│   livenessProbe:                                            │
│     httpGet:                                                │
│       path: /healthz                                        │
│       port: 8080                                            │
│     initialDelaySeconds: 30                                 │
│     periodSeconds: 10                                       │
│     failureThreshold: 3                                     │
│                                                              │
│   readinessProbe:                                           │
│     httpGet:                                                │
│       path: /ready                                          │
│       port: 8080                                            │
│     periodSeconds: 5                                        │
│   ```                                                       │
│                                                              │
│   Deployment gate:                                           │
│   - New pod must pass readiness before old pod terminated  │
│   - Prevents bad deployments from taking down service      │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Rollback Mechanism

```
┌─────────────────────────────────────────────────────────────┐
│              Rollback                                        │
│                                                              │
│   Automatic triggers:                                        │
│   ├── Health check failures exceed threshold               │
│   ├── Error rate spikes                                    │
│   ├── Latency degradation                                  │
│   └── Manual trigger                                       │
│                                                              │
│   Rollback strategies:                                       │
│                                                              │
│   1. Version rollback:                                      │
│      - Keep last N versions available                      │
│      - Deploy previous known-good version                  │
│      - Same deployment process as forward                  │
│                                                              │
│   2. Blue-green rollback:                                   │
│      - Just switch LB back to old environment             │
│      - Instant (<1 second)                                 │
│                                                              │
│   3. Canary rollback:                                       │
│      - Route 100% traffic back to stable version          │
│      - Terminate canary pods                               │
│                                                              │
│   Version storage:                                           │
│   - Container images tagged with commit SHA               │
│   - Config versions in Git (GitOps)                        │
│   - Database migrations must be backward-compatible        │
│                                                              │
│   Command:                                                   │
│   kubectl rollout undo deployment/myapp                     │
│   # or                                                      │
│   kubectl rollout undo deployment/myapp --to-revision=3     │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Configuration Management

```
┌─────────────────────────────────────────────────────────────┐
│              Configuration                                   │
│                                                              │
│   Layers:                                                    │
│   1. Default config (in code)                              │
│   2. Environment config (dev/staging/prod)                 │
│   3. Secret injection (vault, k8s secrets)                 │
│   4. Feature flags (runtime toggles)                       │
│                                                              │
│   GitOps approach:                                           │
│   ├── All config in Git                                    │
│   ├── Pull-based deployment (ArgoCD/Flux)                 │
│   ├── Audit trail via Git history                         │
│   └── Easy rollback (git revert)                          │
│                                                              │
│   Repo structure:                                            │
│   ```                                                       │
│   config/                                                   │
│   ├── base/                                                │
│   │   ├── deployment.yaml                                 │
│   │   └── service.yaml                                    │
│   ├── dev/                                                 │
│   │   └── kustomization.yaml                              │
│   ├── staging/                                             │
│   │   └── kustomization.yaml                              │
│   └── prod/                                                │
│       └── kustomization.yaml                              │
│   ```                                                       │
│                                                              │
│   Secrets:                                                   │
│   ├── Never in Git (use sealed-secrets or external)       │
│   ├── HashiCorp Vault integration                         │
│   ├── AWS Secrets Manager                                  │
│   └── Kubernetes external-secrets                         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### Pipeline Orchestration

```
┌─────────────────────────────────────────────────────────────┐
│              Pipeline Stages                                 │
│                                                              │
│   Full pipeline:                                             │
│                                                              │
│   ┌─────────────────────────────────────────────────────┐   │
│   │ Build │ Test │ Security │ Dev │ Staging │   Prod    │   │
│   │       │      │  Scan    │     │         │           │   │
│   └───┬───┴───┬──┴────┬─────┴──┬──┴────┬────┴─────┬─────┘   │
│       │       │       │        │       │          │         │
│       ▼       ▼       ▼        ▼       ▼          ▼         │
│    [Auto]  [Auto]  [Auto]   [Auto]  [Auto]   [Manual Gate]  │
│                                                              │
│   Environment promotion:                                     │
│   ├── Dev: auto-deploy on merge to main                    │
│   ├── Staging: auto-deploy after dev                       │
│   ├── Prod: manual approval required                       │
│   └── Prod: canary first, then full rollout               │
│                                                              │
│   Quality gates:                                             │
│   ├── All tests pass                                       │
│   ├── Code coverage > 80%                                  │
│   ├── No high/critical vulnerabilities                     │
│   ├── Performance benchmarks met                           │
│   └── Staging smoke tests pass                             │
│                                                              │
│   Approvals:                                                 │
│   ├── Slack/Teams notification                             │
│   ├── Require N approvers                                  │
│   ├── Time-based (no Friday deploys)                       │
│   └── Change window restrictions                           │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

---

## 📊 Summary

```
Key Components:
├── Build Service: Compile, test, create artifacts
├── Deployment Orchestrator: Strategy execution
├── Health Checker: Liveness/readiness monitoring
├── Config Manager: GitOps-based configuration

Key Decisions:
├── Rolling deployment for most cases
├── Canary for high-risk changes
├── Blue-green for instant rollback needs
├── GitOps for config management

Safety Features:
├── Automated rollback on failures
├── Health checks before traffic shift
├── Manual gates for production
├── Audit trail for all changes
```

---

## 📖 Next Steps

→ Continue to [Design ChatGPT](./16-chatgpt.md)
