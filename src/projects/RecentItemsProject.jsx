...
import RecentItemsDiagram from "./RecentItemsDiagram";
import RecentItemsLLD from "./RecentItemsLLD";
import {
  PROJECT_META,
  REQUIREMENTS,
  APIS,
...
// ──────────────────────────────────────
// RecentItemsProject — Full project page
// ──────────────────────────────────────
export default function RecentItemsProject({ onBack }) {
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 -mt-2">
        ...
        <span className="text-xs text-accent font-bold">recent-items-system.md</span>
      ...
      <div className="space-y-3">
        ...
        <p className="text-sm text-comment leading-relaxed">
          A deep dive into the architecture I designed and built at Zoho for the CRM Recent Items feature —
          tracking user activity across 500K DAU with sub-100ms dropdown retrieval via Redis Sorted Sets and Kafka-driven async writes.
        </p>
      </div>
      ...
      <div className="border border-border rounded-md overflow-hidden">
        ...
        <div className="px-1 py-2 text-[10px] text-comment flex items-center gap-2">
          ...
          {activeTab === "hld"
            ? "High-level system architecture with data flow, APIs, and activity consumer deep dive"
            : "Class-level design with Java 17 records, Redis cache patterns, and complexity analysis"}
        </div>
      </div>
      ...
      <Section id="architecture" command="./render_architecture.sh --interactive" defaultOpen={true}>
        <DiagramView
          interactive={<RecentItemsDiagram />}
          staticSrc="/recent_items.png"
          staticAlt="Recent Items System Architecture"
        />
      </Section>
        ...
        <div className="space-y-4">
          ...
          <div className="border-l-2 border-accent/40 pl-3 mt-3">
            <p className="text-xs text-comment leading-relaxed italic">
              ...
              PostgreSQL with hash partitioning chosen because (1) query pattern is simple key-based lookups by user_id,
              (2) need strong consistency for user&apos;s own activity view, (3) 2GB active dataset fits in memory with indexes,
              (4) relational joins may be needed for user profile enrichment. Redis Sorted Sets provide O(log N) inserts
              and O(1) range queries for the read-heavy access pattern.
            </p>
          </div>
        </div>
      </Section>

      {/* ── 4. Activity Consumer Deep Dive ── */}
      <Section id="consumer" command="cat activity_consumer.py --annotated" defaultOpen={true}>
        <div className="space-y-4">
          <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
            ...
            <p className="text-xs text-comment leading-relaxed">
              The Activity Consumer is the bottleneck between high-velocity write traffic (870 writes/sec peak)
              and cache consistency. It must deduplicate rapid repeated actions, maintain exactly 20 items per user
              in Redis, handle backpressure if PostgreSQL slows down, and guarantee eventual consistency between DB and cache.
            </p>
          </div>
          ...
          <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs">
              {[
                ...
                "Batch Buffer (100ms)",
                "Deduplication (user+record)",
                "PostgreSQL Batch Insert",
                "Redis Pipeline (ZADD+ZREM)",
              ].map((step, i, arr) => (
          ...
          <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
            ...
            <ul className="text-xs text-comment leading-relaxed space-y-1">
              {[
                ["Kafka Partitioning", "Partition by user_id hash → guarantees all activities for a user go to same consumer (in-order processing)"],
                ["Consumer Group", "Run 10 consumer instances (matches Kafka partition count) → 870/10 = 87 writes/sec per consumer"],
                ["Batch Processing", "100ms window + deduplication reduces DB writes by ~60% (user clicks 'Save' 3 times → 1 DB write)"],
                ["Redis Pipelining", "50 Redis commands in 1 network round trip → 5x throughput vs. sequential commands"],
              ].map(([label, desc]) => (
          ...
          <CodeBlock lang="PYTHON" title="Activity Consumer Core Logic">
            {CONSUMER_PSEUDOCODE}
          </CodeBlock>
