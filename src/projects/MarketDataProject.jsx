...
import MarketDataLLD from "./MarketDataLLD";
import {
  PROJECT_META,
  REQUIREMENTS,
  APIS,
...
function DataModelTabs() {
  const tabs = [
    { key: "timescaleRaw", label: "TimescaleDB (Raw)", icon: "storage", color: "accent" },
    { key: "timescaleAgg", label: "TimescaleDB (Agg)", icon: "analytics", color: "accent" },
    { key: "redis", label: "Redis", icon: "bolt", color: "keyword" },
    { key: "kafka", label: "Kafka", icon: "stream", color: "success" },
  ];
  const [active, setActive] = useState("timescaleRaw");
...
// ──────────────────────────────────────
// MarketDataProject — Full project page
// ──────────────────────────────────────
export default function MarketDataProject({ onBack }) {
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 -mt-2">
        ...
        <span className="text-xs text-accent font-bold">market-data-aggregation.md</span>
      ...
      <div className="border border-border rounded-md overflow-hidden">
        ...
        <div className="px-1 py-2 text-[10px] text-comment flex items-center gap-2">
          ...
          {activeTab === "hld"
            ? "High-level system architecture with data flow, APIs, and aggregation deep dive"
            : "Class-level design with Java 17 records, concurrency primitives, and complexity analysis"}
        </div>
      </div>
      ...
      <Section id="architecture" command="./render_architecture.sh --interactive" defaultOpen={true}>
        <DiagramView
          interactive={<MarketDataDiagram />}
          staticSrc="/market_data_aggregation.png"
          staticAlt="Market Data Aggregation Architecture"
        />
      </Section>
        ...
        <div className="space-y-4">
          {APIS.map((api) => (
            <div key={api.title} className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {api.request && (
                  <CodeBlock lang="JSON" title="Request">{api.request}</CodeBlock>
                )}
          ...
          <div className="border-l-2 border-accent/40 pl-3 mt-3">
            <p className="text-xs text-comment leading-relaxed italic">
              ...
              TimescaleDB (PostgreSQL extension) chosen because (1) native hypertable partitioning optimizes
              time-series writes (1-hour chunks for raw, 1-day for aggregated), (2) supports complex SQL
              aggregations with ACID guarantees required for financial compliance auditing, (3) continuous
              aggregates auto-compute OHLCV rollups, (4) built-in compression achieves 95% reduction on
              historical data. Redis provides sub-millisecond deduplication lookups and hot-path query caching.
            </p>
          </div>
        </div>
      </Section>

      {/* ── 4. Aggregation Service Deep Dive ── */}
      <Section id="aggregation" command="cat aggregation_service.py --annotated" defaultOpen={true}>
        <div className="space-y-4">
          <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
            ...
            <p className="text-xs text-comment leading-relaxed">
              The Aggregation Service is the bottleneck between 50K raw events/sec ingestion and TimescaleDB writes.
              It must maintain in-memory OHLCV sliding windows per symbol, deduplicate cross-provider data via Redis,
              handle backpressure from TimescaleDB during batch flushes, and guarantee no data loss during window transitions.
            </p>
          </div>
          ...
          <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
            ...
            <ul className="text-xs text-comment leading-relaxed space-y-1">
              {[
                ["Kafka Partitioning", "Partition by symbol hash → guarantees all events for a symbol go to same consumer (in-order OHLCV computation)"],
                ["Consumer Group", "Run 50 consumer instances (matches Kafka partition count) → 50K/50 = 1K events/sec per consumer"],
                ["In-Memory Windowing", "Maintain OHLCV state per (symbol, 1-min bucket) in memory — O(1) update per event, flush only when window closes"],
                ["Batch Writes", "TimescaleDB executemany with ON CONFLICT upsert — ~30K rows per 10-second flush, leveraging hypertable partitioning"],
              ].map(([label, desc]) => (
          ...
          <CodeBlock lang="PYTHON" title="Aggregation Service Core Logic">
            {AGG_PSEUDOCODE}
          </CodeBlock>
