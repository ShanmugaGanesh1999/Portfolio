// ============================================================
// DIAGRAM CANVAS — one parameterized interactive SVG diagram
// that replaces the seven near-identical *Diagram.jsx viewers.
// Pan (drag), zoom (wheel + buttons), click-to-inspect nodes,
// and data-flow path highlighting. Data comes from the existing
// *Data.js files: DIAGRAM_NODES / DIAGRAM_EDGES / DATA_FLOWS.
// ============================================================
...
// ──────────────────────────────────────
function EdgeLine({ edge, highlighted, dimmed, nodesById, markerId }) {
...
// ──────────────────────────────────────
function DiagramNode({ node, isSelected, isHighlighted, isDimmed, onClick, labelFontSize = 11 }) {
  const COLOR_HEX = useThemeColors();
...
/**
 * DiagramCanvas — interactive system-design diagram.
 *
 * @param {Array}  nodes   DIAGRAM_NODES from *Data.js
 * @param {Array}  edges   DIAGRAM_EDGES from *Data.js
 * @param {Object} flows   DATA_FLOWS from *Data.js (keyed object)
 * @param {string} markerId  Unique SVG marker id (avoid <defs> collisions)
 * @param {string} viewBox   e.g. "0 0 1000 680"
 * @param {Object} heights   { mobile: "300px", desktop: "480px" }
 * @param {number} maxZoom   Zoom clamp upper bound (default 3)
 * @param {number} labelFontSize Node label font size (default 11)
 */
export default function DiagramCanvas({
  ...
  markerId,
  viewBox,
  heights = { mobile: "300px", desktop: "480px" },
  maxZoom = 3,
  labelFontSize = 11,
}) {
  ...
  const getNodeById = (id) => nodesById.get(id);
  ...
  const handleWheel = useCallback(
    (e) => {
      ...
      setTransform((prev) => ({
        ...prev,
        scale: Math.max(0.4, Math.min(maxZoom, prev.scale + delta)),
      }));
    },
    [maxZoom]
  );
  ...
  // ── Zoom buttons ──
  const zoomIn = () =>
    setTransform((p) => ({ ...p, scale: Math.min(maxZoom, p.scale + 0.2) }));
  const zoomOut = () =>
    setTransform((p) => ({ ...p, scale: Math.max(0.4, p.scale - 0.2) }));
  const resetView = () => setTransform({ x: 0, y: 0, scale: 1 });

  // ── Highlight computation ──
  const flowNodes = activeFlow ? flows[activeFlow]?.path || [] : [];
  ...
  const isEdgeHighlighted = (edge) => {
    if (activeFlow) {
      const path = flows[activeFlow]?.path || [];
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-1 border border-border rounded-md overflow-hidden">
          <button
            onClick={zoomOut}
            className="px-2 py-1 text-xs text-comment hover:text-text hover:bg-border/50 transition-colors"
            title="Zoom out"
            aria-label="Zoom out"
          >
            −
          </button>
          <span className="text-[10px] text-comment px-2 border-x border-border tabular-nums">
          ...
          <button
            onClick={resetView}
            className="px-2 py-1 text-[10px] text-comment hover:text-text hover:bg-border/50 transition-colors border-l border-border"
            title="Reset view"
          >
            RESET
          </button>
      ...
      <div
        ...
        style={{
          height: isMobile ? heights.mobile : heights.desktop,
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(${COLOR_HEX.text} 1px, transparent 1px), linear-gradient(90deg, ${COLOR_HEX.text} 1px, transparent 1px)`,
      ...
      {(selectedNode || activeFlow) && (
        <div
          className="border border-border rounded-md bg-sidebar/50 p-3 animate-fade-in-up"
        ...
        >
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span
                  ...
                  style={{ color: COLOR_HEX[selectedNode.color] }}
                >
                  {selectedNode.label}
                </span>
          ...
          {activeFlow && !selectedNode && (
            <div className="space-y-1">
              <span
                className="text-sm font-bold"
                style={{ color: COLOR_HEX[flows[activeFlow].color] }}
              >
                {flows[activeFlow].label}
              </span>
              <p className="text-xs text-comment">{flows[activeFlow].description}</p>
            </div>
          )}
        </div>
      )}
      ...
      <p className="text-[10px] text-comment/50 text-center">
        scroll to zoom · drag to pan · click a component to inspect · use flow
        buttons to trace data paths
      </p>
    </div>
  );
}
