...
import TaxConfigDiagram from "./TaxConfigDiagram";
import TaxConfigLLD from "./TaxConfigLLD";
import {
  PROJECT_META,
  REQUIREMENTS,
  APIS,
...
function DataModelTabs() {
  const tabs = [
    ...
    { key: "dynamodb", label: "DynamoDB", icon: "bolt", color: "variable" },
...
// ──────────────────────────────────────
// TaxConfigProject — Full project page
// ──────────────────────────────────────
export default function TaxConfigProject({ onBack }) {
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 -mt-2">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-comment leading-relaxed">
            Architecture deep dive into the multi-tenant Tax Configuration &
            Calculation Service I built at Zoho — powering tax calculations
            across Quotes, Sales Orders, and Invoices for 100K+ organizations
        ...
        <div className="border border-border rounded-md overflow-hidden">
          ...
          <div className="px-1 py-2 text-[10px] text-comment flex items-center gap-2">
            ...
            {activeTab === "hld"
              ? "High-level system architecture with multi-service design, data flow, and tax engine deep dive"
              : "Class-level design with Java 17 records, concurrency primitives, and Strategy Pattern"}
          </div>
        </div>
        ...
        >
          <DiagramView
            interactive={<TaxConfigDiagram />}
            staticSrc="/tax_customization.png"
            staticAlt="Tax Configuration System Architecture"
          />
        </Section>
          ...
          <div className="space-y-4">
            ...
            <div className="border-l-2 border-accent/40 pl-3 mt-3">
              <p className="text-xs text-comment leading-relaxed italic">
                ...
                PostgreSQL chosen for tax configurations because relational
                guarantees (ACID) are critical for financial accuracy, and
                transactional updates to tax rates require strong consistency;
                DynamoDB for API version mappings achieves sub-10ms reads for
                backward compatibility routing without burdening the primary
                database.
              </p>
            </div>
          </div>
        ...
        {/* ── 4. Tax Calc Engine Deep Dive ── */}
        <Section id="engine" command="cat tax_calc_engine.py --annotated">
          <div className="space-y-4">
            ...
            <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
              ...
              <ul className="text-xs text-comment leading-relaxed space-y-1">
                {[
                  [
                    "Horizontal Scaling",
                    "Stateless service with 10 replicas behind ALB; calculation logic is CPU-bound, linear scaling",
                  ],
                  [
                    "Two-Tier Cache",
                    "Redis L1 for configs (30s TTL) and calculation results (5min TTL); reduces DB load by 95%",
                  ],
                  [
                    "Tenant Partitioning",
                    "Configs sharded by org_id in PostgreSQL; DynamoDB auto-scales for API version lookups",
                  ],
                  [
                    "Async Processing",
                    "Kafka decouples audit logging (1-2s delay acceptable) from the calculation critical path",
                  ],
                ].map(([label, desc]) => (
            ...
            <CodeBlock lang="PYTHON" title="Tax Calculation Engine — Core Logic">
              {ENGINE_PSEUDOCODE}
            </CodeBlock>
