...
import RollupSummaryLLD from "./RollupSummaryLLD";
import {
  PROJECT_META,
  REQUIREMENTS,
  APIS,
...
function DataModelTabs() {
  const tabs = [
    ...
    { key: "columnStore", label: "Column Store", icon: "view_column", color: "variable" },
...
// ──────────────────────────────────────
// RollupSummaryProject — Full project page
// ──────────────────────────────────────
export default function RollupSummaryProject({ onBack }) {
  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 -mt-2">
        ...
        <span className="text-xs text-accent font-bold">rollup-summary-system.md</span>
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-comment leading-relaxed">
            A deep dive into the architecture I designed and built at Zoho for the CRM Rollup Summary Field system —
            handling 6,500+ aggregation tasks weekly with sub-100ms latency.
          </p>
        </div>
        ...
        <div className="border border-border rounded-md overflow-hidden">
          ...
          <div className="px-1 py-2 text-[10px] text-comment flex items-center gap-2">
            ...
            {activeTab === "hld"
              ? "High-level system architecture with data flow, APIs, and engine deep dive"
              : "Class-level design with Java 17 records, concurrency patterns, and complexity analysis"}
          </div>
        </div>
        ...
        <Section id="architecture" command="./render_architecture.sh --interactive" defaultOpen={true}>
          <DiagramView
            ...
            staticSrc="/rollup_summary.png"
            staticAlt="Rollup Summary System Architecture"
          />
        </Section>
          ...
          <div className="space-y-4">
            ...
            <div className="border-l-2 border-accent/40 pl-3 mt-3">
              <p className="text-xs text-comment leading-relaxed italic">
                ...
                PostgreSQL chosen for transactional integrity during concurrent child record writes
                and support for complex JOIN-based aggregations with filter criteria; Column Store
                (e.g., ClickHouse) for rollup definitions enables fast metadata lookups across
                100K+ orgs with 10-15 rollups/module.
              </p>
            </div>
          </div>
        </Section>

        {/* ── 4. Rollup Engine Deep Dive ── */}
        <Section id="engine" command="cat rollup_engine.py --annotated" defaultOpen={true}>
          <div className="space-y-4">
            ...
            <div className="border border-border rounded-md bg-sidebar/20 p-3 space-y-2">
              ...
              <ul className="text-xs text-comment leading-relaxed space-y-1">
                {[
                  ["Partitioning", "Kafka partitioned by parent_record_id ensures all events for a parent go to same consumer, avoiding race conditions"],
                  ["Caching", "Rollup definitions cached in-memory (Caffeine) per consumer instance; Redis caches final rollup values"],
                  ["Incremental Updates", "For Sum/Count, apply delta (+/- amount) without re-querying all child records"],
                  ["Async Processing", "Decouples rollup calculation from synchronous API response (child record write returns immediately)"],
                ].map(([label, desc]) => (
            ...
            <CodeBlock lang="PYTHON" title="Rollup Engine Core Logic">
              {ENGINE_PSEUDOCODE}
            </CodeBlock>
