// ============================================================
// GENERIC LLD — one parameterized Low-Level Design page that
// replaces the six near-identical *LLD.jsx viewers. All content
// comes from the existing *LLDData.js files (unchanged contracts):
//   PROBLEM_SCOPE, KEY_APIS, UML_CLASSES, UML_RELATIONSHIPS,
//   CODE_FILES, COMPLEXITY, FOLLOW_UPS, DESIGN_PATTERNS,
//   SOLID_PRINCIPLE
// ============================================================
...
/**
 * GenericLLD — full LLD page.
 *
 * @param {Object} data          LLD data bundle from *LLDData.js
 * @param {Object} tabColors     CODE_FILES filename → color token map
 * @param {string} spaceTitle    Space complexity section heading
 * @param {string} spaceCols     Grid classes for the space section
 * @param {string} entitiesCols  Grid classes for core entities
 * @param {boolean} concurrencyStacked  Stacked list layout (vs grid)
 * @param {string} concurrencyTitle     Concurrency section heading
 * @param {string} apisHeading   API section heading
 */
export default function GenericLLD({
  data,
  tabColors = {},
  spaceTitle = "Space Complexity",
  spaceCols = "sm:grid-cols-2",
  entitiesCols = "sm:grid-cols-2",
  concurrencyStacked = false,
  concurrencyTitle = "Concurrency",
  apisHeading = "Key APIs",
}) {
  const {
    PROBLEM_SCOPE,
    KEY_APIS,
  ...
  // ── Code file tabs (internal state) ──
  const [activeFile, setActiveFile] = useState(Object.keys(CODE_FILES)[0]);
  const file = CODE_FILES[activeFile] ?? Object.values(CODE_FILES)[0];

  // ── Follow-up accordion (internal state) ──
  const [expandedFaq, setExpandedFaq] = useState(null);

  return (
    <div className="space-y-10">
      {/* ── 1. Problem Scope & Assumptions ── */}
      <Section id="lld-scope" command="cat problem_scope.md" defaultOpen={true}>
        <div className="space-y-4">
          ...
          <h3 className="text-sm text-variable font-bold pt-2">Critical Assumptions</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {PROBLEM_SCOPE.assumptions.map((a) => (
              <div key={a.label} className="border border-border rounded-md p-4 bg-sidebar/20">
                <div className="text-xs text-keyword font-bold mb-1">{a.label}</div>
                <p className="text-xs text-comment leading-relaxed">{a.detail}</p>
              </div>
            ))}
          </div>

          <h3 className="text-sm text-variable font-bold pt-2">Core Entities</h3>
          <div className={`grid grid-cols-1 ${entitiesCols} gap-2`}>
            {PROBLEM_SCOPE.coreEntities.map((e) => (
              <div
                key={e.name}
                className="border rounded-md p-4 bg-sidebar/20"
                style={{ borderColor: `var(--color-${e.color})` }}
              >
                <div
                ...
                <p className="text-[11px] text-comment leading-relaxed">{e.description}</p>
              </div>
            ))}
          </div>

          <h3 className="text-sm text-variable font-bold pt-2">{apisHeading}</h3>
          <div className="space-y-2">
            {KEY_APIS.map((api) => (
              <div
                key={api.signature}
              ...
              >
                ...
                <div>
                  <code className="text-xs text-text font-mono">{api.signature}</code>
                  <p className="text-[11px] text-comment mt-0.5">{api.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── 2. UML Class Diagram ── */}
      <Section id="lld-uml" command="./render_uml.sh --class-diagram" defaultOpen={true}>
        <div className="space-y-4">
      ...
      {/* ── 3. Source Code ── */}
      <Section id="lld-code" command="find src/ -type f | xargs cat" defaultOpen={true}>
        <div className="border border-border rounded-md overflow-hidden">
          <div className="flex border-b border-border bg-sidebar/50 overflow-x-auto scrollbar-none">
            {Object.keys(CODE_FILES).map((key) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs border-b-2 transition-colors whitespace-nowrap ${
                    active
                      ? "border-current text-text bg-border/20"
                      : "border-transparent text-comment hover:text-text"
                  }`}
                  style={active ? { color: `var(--color-${color})` } : {}}
                >
          ...
          <pre className="p-3 text-xs leading-relaxed overflow-x-auto bg-sidebar/20 scrollbar-thin">
      ...
      <Section id="lld-complexity" command="cat complexity_analysis.md" defaultOpen={true}>
        <div className="space-y-4">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <div className={`grid grid-cols-1 ${spaceCols} gap-2`}>
              {COMPLEXITY.space.map((item) => (
                <div key={item.component} className="border border-border rounded-md p-3 bg-sidebar/20">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-keyword font-bold">{item.component}</span>
            ...
            {concurrencyStacked ? (
              <div className="space-y-2">
                {COMPLEXITY.concurrency.map((item) => (
                  <div key={item.label} className="border border-border rounded-md p-3 bg-sidebar/20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {COMPLEXITY.concurrency.map((item) => (
                  <div key={item.label} className="border border-border rounded-md p-3 bg-sidebar/20">
                    <span className="text-xs text-success font-bold">{item.label}</span>
                    <p className="text-[11px] text-comment mt-1">{item.detail}</p>
                  </div>
                ))}
              </div>
      ...
      <Section id="lld-followups" command="cat interview_questions.md --top-5" defaultOpen={true}>
        <div className="space-y-2">
          {FOLLOW_UPS.map((faq, i) => (
            <div key={i} className="border border-border rounded-md overflow-hidden transition-all">
              <button
                onClick={() => setExpandedFaq(expandedFaq === i ? null : i)}
                aria-expanded={expandedFaq === i}
                className="w-full text-left px-4 py-3 flex items-start gap-2 hover:bg-border/20 transition-colors"
              >
                <span className="text-accent font-bold text-xs shrink-0 mt-0.5">Q{i + 1}</span>
                <span className="text-xs text-text leading-relaxed flex-1">{faq.question}</span>
                <Icon
                  name={expandedFaq === i ? "expand_more" : "chevron_right"}
                  size="text-sm"
                  className="text-comment shrink-0 mt-0.5"
                />
              </button>
              {expandedFaq === i && (
                <div className="px-4 pb-3 animate-fade-in-up" style={{ animationDuration: "0.2s" }}>
                  <div className="border-l-2 border-success/40 pl-3 ml-5">
                    <p className="text-[11px] text-comment leading-relaxed">
                      <span className="text-success font-bold">A:</span> {faq.answer}
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {DESIGN_PATTERNS.map((dp) => (
              <div key={dp.pattern} className="border border-border rounded-md p-4 bg-sidebar/20">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs text-func font-bold">{dp.pattern}</span>
                  <span className="text-[10px] text-comment font-mono bg-border/30 px-1.5 py-0.5 rounded">
                    {dp.location}
                  </span>
                </div>
                <p className="text-xs text-comment leading-relaxed">{dp.detail}</p>
              </div>
            ))}
          </div>

          <h3 className="text-sm text-variable font-bold pt-2">SOLID Principle</h3>
          <div className="border-l-4 border-accent/40 pl-4 bg-sidebar/20 p-4 rounded-r-md">
            <div className="text-xs text-accent font-bold mb-1">{SOLID_PRINCIPLE.principle}</div>
            <p className="text-xs text-comment leading-relaxed">{SOLID_PRINCIPLE.detail}</p>
          </div>
        </div>
      </Section>
    </div>
